...
const WARNING_SIGNS = [
  {
    ...
    icon: (p: any) => (<><circle cx="12" cy="12" r="9" {...p} /><polyline points="12 7 12 12 15 14" {...p} /></>),
  },
  {
    title: 'The dryer feels very hot',
    desc: 'A hot dryer or laundry room means heat has nowhere to escape.',
    icon: (p: any) => (<path d="M12 2s5 5.5 5 10a5 5 0 0 1-10 0c0-4.5 5-10 5-10z" {...p} />),
  },
  {
    title: 'A burning or musty smell',
    desc: 'Scorched lint smells hot. Trapped moisture smells musty. Both are bad.',
    icon: (p: any) => (<><path d="M8 20h8" {...p} /><path d="M10 20c-2-2-3-4-1-7 1 2 2 2 3 1 1-2 0-4-1-6 4 1 6 5 6 8a5 5 0 0 1-7 4z" {...p} /></>),
  },
  {
    title: 'Lint around the vent opening',
    desc: 'Visible lint outside means it is packing up on the inside too.',
    icon: (p: any) => (<><rect x="3" y="4" width="18" height="16" rx="2" {...p} /><circle cx="12" cy="12" r="3.5" {...p} /></>),
  },
  {
    title: "It's been over a year",
    desc: 'Vents should be cleaned yearly — sooner for big families or pets.',
    icon: (p: any) => (<><rect x="3" y="5" width="18" height="16" rx="2" {...p} /><line x1="3" y1="10" x2="21" y2="10" {...p} /><line x1="8" y1="3" x2="8" y2="7" {...p} /><line x1="16" y1="3" x2="16" y2="7" {...p} /></>),
  },
  {
    title: 'The vent flap stays shut',
    desc: 'If the outside flap barely opens, airflow is badly blocked.',
    icon: (p: any) => (<><path d="M4 12h16" {...p} /><path d="M4 12l4-4M4 12l4 4" {...p} /></>),
  },
];
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1080px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: isMobile ? '32px' : '48px' }}>
          ...
          <h2 style={{
            ...
            color: '#111111', margin: '0 0 14px', lineHeight: 1.08,
          }}>
            Is Your Dryer a Fire<br />Waiting to Happen?
          </h2>
          <p style={{
            fontSize: isMobile ? '0.95rem' : '1.1rem', fontWeight: 500,
            color: '#555555', maxWidth: '640px', margin: '0 auto', lineHeight: 1.7,
          }}>
          ...
          {STATS.map((s, i) => (
            <div key={s.label} style={{
              ...
              <div style={{
                ...
                color: '#FFD400', letterSpacing: '-0.02em',
              }}>{s.num}</div>
              <div style={{
            ...
            {[
              ...
              <div key={v.src} style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    display: 'inline-block', background: '#FFD400', color: '#111',
                    fontFamily: "var(--font-oswald), sans-serif", fontWeight: 700,
            ...
            {WARNING_SIGNS.map(w => (
              <div key={w.title} style={{
                ...
                <div style={{
                  ...
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FFD400" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    {w.icon({})}
                  </svg>
                </div>
                <div>
                  <h4 style={{ fontFamily: "var(--font-inter), sans-serif", fontWeight: 800, fontSize: '0.98rem', color: '#111111', margin: '0 0 4px' }}>{w.title}</h4>
                  <p style={{ fontSize: '0.82rem', color: '#555555', lineHeight: 1.55, margin: 0 }}>{w.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ marginBottom: isMobile ? '38px' : '52px' }}>
          <h3 style={{
            textAlign: 'center', fontFamily: "var(--font-inter), sans-serif", fontWeight: 900,
            fontSize: isMobile ? '1.4rem' : '1.9rem', color: '#111111', margin: '0 0 24px',
          }}>
            Why It&apos;s Worth It
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: isMobile ? '10px' : '14px' }}>
            {BENEFITS.map(b => (
              <div key={b.title} style={{
                ...
                borderRadius: '10px', padding: isMobile ? '14px 16px' : '18px 20px',
                display: 'flex', alignItems: 'flex-start', gap: '12px',
              }}>
                <div style={{ flexShrink: 0, width: '30px', height: '30px', borderRadius: '50%', background: '#FFD400', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1.5px solid #1a1a1a' }}>
                  <BenefitCheck />
                </div>
                <div>
                  <h4 style={{ fontFamily: "var(--font-inter), sans-serif", fontWeight: 800, fontSize: '0.95rem', color: '#111111', margin: '0 0 3px' }}>{b.title}</h4>
                  <p style={{ fontSize: '0.82rem', color: '#555555', lineHeight: 1.5, margin: 0 }}>{b.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{
          ...
          border: '1.5px solid rgba(255,212,0,0.35)',
        }}>
          <h3 style={{
          ...
          <p style={{ fontSize: isMobile ? '0.9rem' : '1rem', color: 'rgba(255,255,255,0.72)', margin: '0 0 22px', lineHeight: 1.6, maxWidth: '520px', marginLeft: 'auto', marginRight: 'auto' }}>
            Professional dryer vent cleaning takes under an hour and starts at just <span style={{ color: '#FFD400', fontWeight: 800 }}>$69</span>. Book today and breathe easy.
          </p>
          <a href="tel:+14252005790" style={{
            ...
            textDecoration: 'none', boxShadow: '0 8px 30px rgba(255,212,0,0.45)',
          }}>
