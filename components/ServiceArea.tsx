...
const HIGHLIGHT_ICONS = {
  home: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#FFD400" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function LeafletMap() {
  ...
  useEffect(() => {
    ...
    const ensureLeaflet = (): Promise<any> => {
      ...
      if (!document.querySelector('link[data-momo-leaflet]')) {
        const css = document.createElement('link');
        css.rel = 'stylesheet';
        css.href = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css';
        css.setAttribute('data-momo-leaflet', '1');
        document.head.appendChild(css);
      }
      return new Promise((resolve, reject) => {
        const existing = document.querySelector('script[data-momo-leaflet]') as HTMLScriptElement | null;
        if (existing) {
        ...
        js.async = true;
        js.setAttribute('data-momo-leaflet', '1');
    ...
    const makeIcon = (major: boolean) => {
      const size = major ? 22 : 16;
      const half = size / 2;
      return L.divIcon({
        className: '',
        html: `<div style="position:relative;width:${size}px;height:${size}px">
          ${major ? `<div style="position:absolute;inset:-5px;border-radius:50%;border:2.5px solid rgba(255,212,0,0.7);animation:momo-ping 2s ease-out infinite"></div>` : ''}
          <div style="width:${size}px;height:${size}px;background:radial-gradient(circle at 38% 35%,#FFE566,#FFD400 60%,#e06b00);border:${major ? 3.5 : 2.5}px solid #fff;border-radius:50%;box-shadow:0 0 0 ${major ? 3 : 2}px rgba(255,212,0,0.6),0 3px 12px rgba(0,0,0,0.35)"></div>
        </div>`,
        iconSize: [size, size],
    ...
    <section style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1080px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '52px' }}>
          ...
          <h2 style={{ fontWeight: 900, fontSize: 'clamp(2rem, 4vw, 3rem)', color: '#111', fontFamily: "var(--font-fraunces), serif", margin: '0 0 14px', lineHeight: 1.1 }}>
            Our Service Area
          </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {HIGHLIGHTS.map(h => (
                <div key={h.label} style={{
                  ...
                  <span style={{ flexShrink: 0, display: 'flex', alignItems: 'center' }}>{HIGHLIGHT_ICONS[h.icon as keyof typeof HIGHLIGHT_ICONS]}</span>
              ...
              <div style={{
                ...
                <a href="tel:+14252005790" style={{
                  ...
                  color: '#FFD400', textDecoration: 'none',
                }}>
                  (425) 200-5790 →
                </a>
              </div>
